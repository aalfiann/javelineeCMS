...
if (isset($_POST['save_cfooter'])) {
    if (get_magic_quotes_gpc()) {
        ...
        $process = $_POST['content'];
        $fp = fopen("../cfooter.php", "w");
        fwrite($fp, $process);
        fclose($fp);
            echo "<script language=\"javascript\">";
            echo "alert(\"".$lang['success_editing']." Custom ".$lang['footer']."\");";
            echo "window.location.href = \"edit.cfooter.php\";";         
            echo "</script>";
    }
...
// Section Body JS Custom
if (isset($_POST['save_ccomponent'])) {
    if (get_magic_quotes_gpc()) {
            $process = array(&$_GET, &$_POST, &$_COOKIE, &$_REQUEST);
            while (list($key, $val) = each($process)) {
                foreach ($val as $k => $v) {
        ...
        $process = $_POST['content'];
        $fp = fopen("../cbody.js.php", "w");
        fwrite($fp, $process);
        fclose($fp);
            echo "<script language=\"javascript\">";
            echo "alert(\"".$lang['success_editing']." Custom ".$lang['component']."\");";
            echo "window.location.href = \"edit.cbody.js.php\";";            
            echo "</script>";
    }
