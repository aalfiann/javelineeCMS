...
function showPagination($table, $limit = 10)
{
	$table = 'advertise';
	$limit = 10;
    $countTotalRow = mysql_query('SELECT COUNT(*) AS total FROM '.$table.' ORDER BY expired_at ASC');
...
function getTableData($table, $page = 1, $limit = 10)
{
    ...
    $query = mysql_query('SELECT a.name_ads,a.created_at,a.expired_at,
		a.harga_ads,b.detail,a.view_ads, a.status_ads,a.code_ads
		FROM advertise a
		INNER JOIN advertise_type b on a.type_ads = b.type_ads
		ORDER BY a.expired_at ASC LIMIT '.$startRow.', '.$limit);
...
<?php function tableads(){
	global $table,$totalrow,$dataPerPage,$page,$dataTable;
	$query = "SELECT a.name_ads,a.created_at,a.expired_at,
		a.harga_ads,b.detail,a.view_ads, a.status_ads,a.code_ads
		FROM advertise a
		INNER JOIN advertise_type b on a.type_ads = b.type_ads
...
function ViewAds(){
	global $totalrow,$result,$noPage,$jumData,$adshasil;
...
// jumlah data yang akan ditampilkan per halaman
	...
	$query = "SELECT *
		FROM advertise a
		INNER JOIN advertise_type b on a.type_ads = b.type_ads
		ORDER BY a.expired_at ASC LIMIT $offset, $dataPerPage";
	$result = mysql_query($query);
	if (!$result) IsQueryError(); 

	// mencari jumlah semua data dalam tabel 
	$query2   = "SELECT COUNT(*) AS jumData FROM advertise ORDER BY expired_at ASC";
	...
	//Menentukan jenis tipe iklan
	$adsquery = "SELECT * FROM advertise_type";
	$adshasil = mysql_query($adsquery);
}