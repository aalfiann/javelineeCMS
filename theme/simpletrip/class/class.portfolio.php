		...
		public static function Load($item){
			// Default show portfolio
				...
				while($datafolio = mysql_fetch_array($portfolio)){
    					...
    					echo'
    					<article class="item">
    						<a href="'.$host.$datafolio['url_img'].'" title="'.stripcslashes($datafolio['t_img']).'" class="image fit" target="_blank">
    						<img src="'.$host.$datafolio['url_img'].'" alt="'.stripcslashes($datafolio['alt_img']).'" /></a>
    						<header>
								<h3>'.stripcslashes($datafolio['t_img']).'</h3>
							</header>
    					</article>';

	            } echo "</center></noscript>";

    	        // Output button Loadmore
        	    echo'
            	<br><div class="12u">
					<input type="submit" value="Load More" id="load_more_button" class="load_more"/>
		...
		public static function ShowResult(){
			//output results protfolio from loadmore button
				...
				while($datafolio = mysql_fetch_array($portfolio)){
    				...
    				echo '
    					<article class="item">
    						<a href="'.$host.$datafolio['url_img'].'" target="_blank" class="image fit">
    						<img src="'.$host.$datafolio['url_img'].'" alt="'.stripcslashes($datafolio['alt_img']).'" /></a>
    						<header>
    							<h3>'.stripcslashes($datafolio['t_img']).'</h3>
    						</header>
