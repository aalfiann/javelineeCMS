...
function TagList() {
	...
	if (mysql_num_rows($hasilhash) > 0) {
									while($row = mysql_fetch_row($hasilhash)) { 
										...
										$more = explode("<!--more-->", $row[2]); echo $more[0]; echo "<a href=\"$_SERVER[PHP_SELF]?p=read&id=$row[0]&s=$row[5]\"><h5>$lang[read_more]</h5></a>";
...
if(isset($_GET['p'])) {if($_GET['p'] == "read"){
	...
	$hasil = mysql_query($query);
	$data  = mysql_fetch_array($hasil);
		if ($data['t_post'] == null) header("Location: $_SERVER[PHP_SELF]?p=error");
	}
}
...
function DataPage(){
	...
	if ($data['t_page'] == null) header("Location: $_SERVER[PHP_SELF]?p=error");
		echo "<h1 class=\"page-header\"><a href=\"".$host."?p=page&id=".$data['id_page']."&s=".$data['slug']."\">".stripcslashes($data['t_page'])."</a>";
                echo "</h1>";
...
function comment() {
	...
	echo "<script type=\"text/javascript\">";
    	...
    	echo "var disqus_url = '".$host."?p=read&id=".$data['id_post']."&s=".$data['slug']."';";  
        echo "(function() {";
...
function GetPortfolio() {
	global $queryfolio,$datafolio,$portfolio;
...
function BreadRead() {
	global $root,$category,$host,$data;
	...
	echo "<li itemprop=\"child\" itemscope itemtype=\"http://data-vocabulary.org/Breadcrumb\"><a href=\"".$_SERVER['PHP_SELF']."?p=post\" itemprop=\"url\"><span itemprop=\"title\">".$lang['posts']."</span></a></li>";
	echo "<li itemprop=\"child\" itemscope itemtype=\"http://data-vocabulary.org/Breadcrumb\"><a href=\"".$_SERVER['PHP_SELF']."?p=cat&s=".$category."\" itemprop=\"url\"><span itemprop=\"title\">".$category."</span></a></li>";
...
function BreadTag() {
	...
	include $root.'/acp/model/set.lang.php';
	echo "<ol class=\"breadcrumb\">";
	echo "<li itemscope itemtype=\"http://data-vocabulary.org/Breadcrumb\"><i class=\"fa fa-home\"></i> <a href=\"".$host."\" itemprop=\"url\"><span itemprop=\"title\">Home</a></li>";
	echo "<li itemprop=\"child\" itemscope itemtype=\"http://data-vocabulary.org/Breadcrumb\"><a href=\"".$_SERVER['PHP_SELF']."?p=post\" itemprop=\"url\"><span itemprop=\"title\">".$lang['posts']."</span></a></li>";
...
function FrontMeta(){
	...
	echo "<title>".$sseo['front_meta_title']."</title>";
...
function HashtagMeta(){
	...
	include $root.'/acp/model/set.lang.php';
	if (!empty($_GET['page'])){
		echo "<title>".stripcslashes($sseo['hashtag_meta_title'])." | ".$_GET['hash']." ".$lang['page']." ".((empty($_GET['page']))? '1' : $_GET['page'])."</title>";
		echo "<meta name=\"keywords\" content=\"".stripcslashes($sseo['hashtag_meta_keywords']).", ".$_GET['hash']." ".$lang['page']." ".((empty($_GET['page']))? '1' : $_GET['page'])."\">";
		echo "<meta name=\"description\" content=\"".stripcslashes($sseo['hashtag_meta_description'])." | ".$_GET['hash']." ".$lang['page']." ".((empty($_GET['page']))? '1' : $_GET['page'])."\">";		
	}
	else {echo "<title>".stripcslashes($sseo['hashtag_meta_title'])." | ".$_GET['hash']."</title>";
		echo "<meta name=\"keywords\" content=\"".stripcslashes($sseo['hashtag_meta_keywords']).", ".$_GET['hash']."\">";
		echo "<meta name=\"description\" content=\"".stripcslashes($sseo['hashtag_meta_description'])." | ".$_GET['hash']."\">";
	}
}
...
function CategoryMeta(){
	...
	else {echo "<title>".stripcslashes($sseo['category_meta_title'])." | ".$_GET['s']."</title>";
...
function AutoHashtag($datatext){
	global $host,$root;
	$regex = "/(#)+[a-zA-Z0-9]+/";
	$datatext = preg_replace($regex, '<a href="'.$_SERVER['PHP_SELF'].'?p=tag&hash=$0">$0</a>', $datatext);
	$remove = array("#");
